...
import os, math, datetime as dt, json, hashlib, asyncio, re
...
# Networking
HTTP_TIMEOUT = float(os.getenv("HTTP_TIMEOUT", "10"))  # seg por request
...
# ---------- Data fetching (async) ----------
async def fetch_h2h_odds_async(client: httpx.AsyncClient, sport_key: str) -> List[Match]:
...
async def fetch_all_matches_async() -> List[Match]:
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        tasks = []
        for lg in LEAGUES:
            tasks.append(fetch_h2h_odds_async(client, lg))
...
def match_to_pick(m: Match) -> Optional[dict]:
    ...
    return {"league": m.league, "date": dstr, "home": m.home, "away": m.away, "side": side, "price": bp}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Hola 👋 Soy tu bot de picks (memoria Redis, async).\n"
        "Comandos:\n"
...
async def reply_picks(update: Update, day: dt.date, label: str):
    ...
    if not ODDS_API_KEY:
        await update.message.reply_text("Falta ODDS_API_KEY. Configúrala en el servicio y vuelve a intentar.")
        return
